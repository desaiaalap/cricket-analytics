...
class TestLoadYaml(unittest.TestCase):
    ...
    def test_parse_match_info_teams(self):
        ...
        self.assertEqual(len(info['teams'].split(' vs ')), 2)
        self.assertIn('Team A', info['teams'])
        self.assertIn('Team B', info['teams'])

    def test_parse_match_info_outcome(self):
        """Test parsing match outcome"""
        info = parse_match_info(self.test_yaml)

        self.assertEqual(info['winner'], 'Team A')
...
class TestParseMatch(unittest.TestCase):
    ...
    def test_parse_match_columns(self):
        ...
        expected_columns = ['innings', 'over', 'ball', 'batting_team', 'batsman',
    ...
    def test_parse_match_data_integrity(self):
        ...
        self.assertEqual(first_delivery['runs_batsman'], 4)
    ...
    def test_parse_match_empty_innings(self):
        ...
        empty_match = {
            'info': {'venue': 'Test Stadium', 'teams': ['A', 'B']},
            'innings': []
        }
...
class TestDataTypeValidation(unittest.TestCase):
    ...
    def test_over_and_ball_numbers(self):
        """Test that over and ball numbers are properly extracted"""
        test_match = {
            'info': {'venue': 'Test', 'teams': ['A', 'B']},
            'innings': [{
                'team': 'A',
        ...
        self.assertEqual(df.iloc[0]['over'], 5)
        self.assertEqual(df.iloc[0]['ball'], 3)
