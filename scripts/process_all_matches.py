...
boundaries = (
    ...
    .apply(
        lambda x: pd.Series(
            {"fours": (x["runs_batter"] == 4).sum(), "sixes": (x["runs_batter"] == 6).sum()}
        ),
...
bowling_stats["average"] = bowling_stats.apply(
    lambda row: (
        round(row["runs_conceded"] / row["wickets"], 2) if row["wickets"] > 0 else float("inf")
    ),
    axis=1,
)
bowling_stats["strike_rate"] = bowling_stats.apply(
    lambda row: (
        round(row["balls_bowled"] / row["wickets"], 2) if row["wickets"] > 0 else float("inf")
    ),
    axis=1,
)
